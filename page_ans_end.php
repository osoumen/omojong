...
foreach ( $members as $memb ) {
	if ( $memb != $myname ) {
		if ( $use_useraccount_for_mension ) {
			$error = commit_mention( $memb, $notifymsg2 . $session['session_key'] . $hash_tag, $_SESSION['access_token']['oauth_token'],$_SESSION['access_token']['oauth_token_secret']);
		}
		else {
			$error = commit_mention( $memb, $notifymsg2 . $session['session_key'] . $hash_tag );
		}
	}
	if ( $error ) {
